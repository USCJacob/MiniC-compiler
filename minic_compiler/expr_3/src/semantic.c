...
{ //处理变量列表
    ...
    if (T)
    {
        ...
        case ID:
            newAlias(alias);
            rtn = fillSymbolTable(T->type_id, alias, LEV, T->type, 'V', T->offset); //最后一个变量名
            if (rtn == -1)
    ...
    if (T)
    {
        ...
        case OR:
            ...
            if (T->kind == AND)
            {
                ...
                strcpy(T->ptr[0]->Etrue, label);
                strcpy(T->ptr[0]->Efalse, T->Efalse);
            }
            else
            {
                strcpy(T->ptr[0]->Etrue, T->Etrue);
                strcpy(T->ptr[0]->Efalse, label);
            }
...
void Exp(struct ASTNode *T)
{
    ...
    if (T)
    {
        ...
        case INT:
            newTemp(temp);
            T->place = fill_Temp(temp, LEV, T->type, 'T', T->offset); //为整常量生成一个临时变量
            T->type = INT;
            opn1.kind = INT;
...
void func_dec(struct ASTNode *T)
{
    ...
    struct opn opn1, opn2, result;
    newAlias(alias);
    rtn = fillSymbolTable(T->type_id, alias, LEV, T->type, 'F', 0); //函数不在数据区中分配单元，偏移量为0
    if (rtn == -1)
    {
...
void param_dec(struct ASTNode *T)
{
    ...
    struct opn opn1, opn2, result;
    newAlias(alias);
    rtn = fillSymbolTable(T->ptr[1]->type_id, alias, 1, T->ptr[0]->type, 'P', T->offset);
    if (rtn == -1)
